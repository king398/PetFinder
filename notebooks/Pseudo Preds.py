...
class CuteDataset(Dataset):
	def __init__(self, images_filepaths, transform=None, transform_flip=None,
	...
	def __getitem__(self, idx):
		...
		image_filepath = image_filepath.split("\\")[2]
		image_filepath = image_filepath.split(".")[0]
		...
		return image1, image2, image3, image4, image_filepath
...
for zx in range(0, 10):
	...
	test_dataset = CuteDataset(
		images_filepaths=glob.glob("E:\Pseudo Dogs/*.jpg"),
	...
	with torch.no_grad():
		for (images, images_flip, images_shift, images_rotate, id) in tqdm(val_loader,
		                                                                   desc=f'Predicting. '):
			...
			for i, x in zip(predictions, id):
				if i < 23 or i > 70:

					preds.append(i)
					ids.append(id)

oof_csv = {"id": ids, "pred": preds}

oof = pd.DataFrame.from_dict(oof_csv)
oof.to_csv(rf"F:\Pycharm_projects\PetFinder\oof files\{params['model']}_oof_crop_pseudo.csv", index=False)
