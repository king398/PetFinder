...
class PetNet(nn.Module):
	def __init__(self, model_1_name=params['model_1'], model_2_name=params['model_2'],
		...
		self.attention = TripletAttention(n_features_2,
		                                  act_layer=act_layer,
		                                  kernel_size=13)
		self.head_conv = nn.Linear(n_features_2, 128)
		self.model_2.reset_classifier(0, '')
	...
	def forward(self, image, dense):
		...
		conv_embeddings = self.attention(conv_embeddings)
		conv_embeddings = self.head_conv(conv_embeddings)
