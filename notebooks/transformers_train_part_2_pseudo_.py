# -*- coding: utf-8 -*-
"""transformers_train_part_2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1V3shd2yjOGSdkzQbFzXRpy7kL0FJG53J
...
def train_fn(train_loader, model, criterion, optimizer, epoch, params, pseudo_loader, scheduler=None):
	...
	for x, images in enumerate(stream_pseudo):
		...
		writer.add_scalar("unlabeled_loss/train", unlabeled_loss, epoch)
		optimizer.step()
		...
		if x % 1000 == 0:
			writer.add_images(f"{output.sigmoid() * 100}", images)
			print(unlabeled_loss)
			print(
				f"output of the model when in train {output[3].sigmoid()} eval model output {output_unlabeled[3].sigmoid()}")
		if x % 2000 == 0:
			for i, (images, dense, target) in enumerate(stream, start=1):
				...
				writer.add_scalar("Labelled Loss/train", unlabeled_loss, x)
				rmse_score = usr_rmse_score(output, target)
				writer.add_scalar("Labelled RMSE/train", rmse_score, x)
...
def validate_fn(val_loader, model, criterion, epoch, params):
	...
	with torch.no_grad():
		for i, (images, dense, target) in enumerate(stream, start=1):
			...
			loss = criterion(output, target)
			rmse_score = usr_rmse_score(output, target)
			writer.add_scalar("Labelled Loss/valid", loss, epoch)
			writer.add_scalar("Labelled RMSE/valid", rmse_score, epoch)
			metric_monitor.update('Loss', loss.item())
			metric_monitor.update('RMSE', rmse_score)
...
for fold in TRAIN_FOLDS:
	...
	pseudo_dataset = CuteDatasetPseudo(images_filepaths=glob.glob("./"),
	                                   transform=get_train_transforms())
...
!cp - r / content / swin_large_patch4_window12_384_2_epoch_f7_17
.689
_rmse.pth / content / swin_large_patch4_window12_384_3_epoch_f8_17
.183
_rmse.pth / content / drive / MyDrive / models / SwinLarge384Withcrop
